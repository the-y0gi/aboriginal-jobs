...
function PasswordStrength({ password }: { password?: string }) {
  ...
  return (
    <div className="mt-2 text-xs flex items-center gap-1.5 font-medium">
      <div className={`w-2 h-2 rounded-full ${lengthValid ? 'bg-green-500' : 'bg-amber-500'}`} />
      <span className={lengthValid ? 'text-green-600' : 'text-amber-600'}>
        {lengthValid ? 'Strong password format' : 'Password must be at least 8 characters'}
      </span>
    </div>
  );
}
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-center gap-2 mb-6 select-none">
                  {['Email', 'OTP', 'Reset'].map((label, idx) => {
                    const stepNum = idx + 1;
                    const isActive = (step === 'email' && stepNum === 1) || (step === 'otp' && stepNum === 2) || (step === 'reset' && stepNum === 3);
                    const isDone = (step === 'otp' && stepNum === 1) || (step === 'reset' && (stepNum === 1 || stepNum === 2));
                    
                    return (
                      <div key={label} className="flex items-center">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                          isDone ? 'bg-[#7A9E7E] text-white shadow-sm' : isActive ? 'bg-[#C8782A] text-white shadow-md' : 'bg-[#C8782A]/10 text-[#6B3A2A]/40'
                        }`}>
                          {isDone ? <CheckCircle size={14} /> : stepNum}
                        </div>
                        {idx < 2 && <div className={`w-10 h-0.5 rounded-full mx-1 ${isDone ? 'bg-[#7A9E7E]' : 'bg-[#C8782A]/15'}`} />}
                      </div>
                    );
                  })}
                </div>
                ...
                {step === 'email' && (
                  <div className="space-y-4">
                    <div>
                      <Label className="text-[#6B3A2A] font-bold text-sm tracking-wide">Email Address</Label>
                      <Input
                        type="email"
                        value={email}
                    ...
                    >
                      {loading ? (
                        ...
                        <span className="flex items-center gap-2 justify-center">Send Verification Code <Mail size={15} /></span>
                      )}
                    </Button>
                  </div>
                )}
                ...
                {step === 'otp' && (
                  <div className="space-y-4 animate-in fade-in duration-300">
                    <div className="text-center bg-[#FAF5EE]/60 rounded-xl py-2.5 px-3 border border-[#C8782A]/5">
                      <p className="text-xs font-medium text-[#6B3A2A]/70">We sent a verification code to</p>
                      <p className="text-sm font-bold text-[#C8782A] break-all mt-0.5">{email}</p>
                    </div>
                    
                    <div className="flex flex-col items-center justify-center my-5">
                      <InputOTP maxLength={6} value={otp} onChange={setOtp} disabled={loading}>
                        <InputOTPGroup>
                          <InputOTPSlot index={0} className="border-[#C8782A]/20 focus:border-[#C8782A] w-11 h-12 text-base rounded-l-xl" />
                          <InputOTPSlot index={1} className="border-[#C8782A]/20 focus:border-[#C8782A] w-11 h-12 text-base" />
                          <InputOTPSlot index={2} className="border-[#C8782A]/20 focus:border-[#C8782A] w-11 h-12 text-base rounded-r-xl" />
                        </InputOTPGroup>
                        <InputOTPSeparator className="text-[#C8782A]/40 mx-1" />
                        <InputOTPGroup>
                          <InputOTPSlot index={3} className="border-[#C8782A]/20 focus:border-[#C8782A] w-11 h-12 text-base rounded-l-xl" />
                          <InputOTPSlot index={4} className="border-[#C8782A]/20 focus:border-[#C8782A] w-11 h-12 text-base" />
                          <InputOTPSlot index={5} className="border-[#C8782A]/20 focus:border-[#C8782A] w-11 h-12 text-base rounded-r-xl" />
                        </InputOTPGroup>
                      </InputOTP>
                    </div>
                    ...
                    <div className="text-center min-h-[24px]">
                      {otpCountdown > 0 ? (
                        <p className="text-xs font-medium text-[#6B3A2A]/50">
                          Resend code in <span className="font-bold text-[#6B3A2A]">{otpCountdown}s</span>
                        </p>
                      ) : (
                        <button
                          type="button"
                    ...
                    {devOtp && (
                      <div className="bg-[#FAF5EE] border border-[#C8782A]/15 rounded-xl p-3 text-center">
                        <p className="text-[10px] font-bold text-[#C8782A] uppercase tracking-wider mb-0.5">🛠️ Dev Sandbox</p>
                        <p className="text-xs text-[#6B3A2A]/70 font-medium">Auto-generated OTP: <span className="text-[#C8782A] font-mono text-sm font-bold tracking-wider">{devOtp}</span></p>
                      </div>
                    )}
                ...
                {step === 'reset' && (
                  <div className="space-y-4 animate-in fade-in duration-300">
                    <div>
                      <Label className="text-[#6B3A2A] font-bold text-sm tracking-wide">New Password</Label>
                    ...
                    <div>
                      <Label className="text-[#6B3A2A] font-bold text-sm tracking-wide">Confirm New Password</Label>
                      <div className="relative mt-1.5">
                        <Input
                    ...
                    <Button
                      ...
                      className="w-full h-11 bg-[#C8782A] hover:bg-[#B06820] text-white font-semibold rounded-xl shadow-md shadow-[#C8782A]/10 transition-all cursor-pointer"
                    >
                      {loading ? 'Updating Password...' : <span className="flex items-center gap-2 justify-center">Reset Password <Lock size={15} /></span>}
                    </Button>
                  </div>
                )}
              </div>
